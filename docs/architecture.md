...
| 분리 산출물 | 물리 인프라 구성은 `infrastructure.md` (No.8) 참조 |

> 본 문서는 **논리 아키텍처·배포 구조·보안 설계**를 다룬다. 물리적 서버·DB·네트워크 구성은 별도 산출물(No.8).
...
│  · *Mapper (MyBatis, XML)                        │
│  · External Adapter (이메일 발송, AI 개발 호출,    │
│    스토리지, 감사 로그 등)                         │
└──────────────────────────────────────────────────┘
```
...
| **dev** | AI 자동 개발 큐 등록·진행 상태·결과 수신 | REQ-DEV-001 |
...
- 외부 호출(이메일·AI 개발 큐) 포함 시 **트랜잭션 외부**에서 발행 (이벤트 발행 패턴)

### 4-4. 비동기 처리

- AI 자동 개발(REQ-DEV-001): 비동기 큐 등록, 백그라운드 워커가 처리
